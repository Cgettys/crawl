...
void monster::banish(const actor *agent, const string &, const int, bool force)
{
    ...
    if (!cell_is_solid(old_pos))
        place_cloud(cloud_type::TLOC_ENERGY, old_pos, 5 + random2(8), 0);
    for (adjacent_iterator ai(old_pos); ai; ++ai)
        if (!cell_is_solid(*ai) && !cloud_at(*ai) && coinflip())
            place_cloud(cloud_type::TLOC_ENERGY, *ai, 1 + random2(8), 0);
...
bool monster::do_shaft()
{
    ...
    place_cloud(cloud_type::DUST, pos(), 1 + random2(3), this);
                               ...
                               beam_type flavour, kill_method_type ktype)
{
        ...
        && has_ench(ENCH_INNER_FLAME) && oppressor && damage)
    {
        ...
        check_place_cloud(cloud_type::FIRE, pos(), 3, actor_by_mid(i_f.source));
    }
    ...
    {
        place_cloud(cloud_type::FIRE, pos(), 20 + random2(15), oppressor, 5);
    }
